...
If `init.luau` were only used for packages or library entry points, I'd be more content with it. But when the pattern encourages the entire codebase to become a nesting doll of init files, the abstraction tower collapses.
...
Why is this a bad thing? Because it introduces a special case the RFC itself admits needs guardrails.
...
This double standard is not only confusing, but it also creates a weird inconsistency in the language. You have to be aware of which type of module you're in to know how to write your `require` statements.
...
While Azul isn't nearly as big as Rojo, it still has a decent userbase _(if 70 stars on GitHub are anything to go by!)_ and it's used in some pretty big projects, so these changes still have an impact on a small side of the community.
