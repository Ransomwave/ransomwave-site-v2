...
The main problem with the pattern is that you lose context. When asking yourself what a file does, `init.luau` tells you too look elsewhere.
...
_(This is the codebase for the Rojo plugin, which otherwise is a pretty well-maintained and organized codebase.)_
...
Here, you're stripped of the context `init.luau` delegates to the directory.

Which `init.luau` is which? You have to hover over each tab to see the file path, and even then, it's super frustrating to manually distinguish between them.

> As an aside, I'm aware that there are certain settings in popular IDEs like VSCode that can help mitigate this issue. But the fact remains: _this is a workaround_. Fundamentally, the problem isn't solved, which is why I will be ignoring these settings for the sake of this discussion.
...
Azul has been using this system for a while now, and I can confidently say that I've personally been much more happier with it. It's much more intuitive, it's easier to navigate, and it doesn't create any of the problems that the init pattern does.

# Conclusion

The `init.luau` pattern was a well-intentioned solution that came from necessity, but it introduces structural ambiguity that we shouldn't accept anymore. It creates more problems than it solves, and it forces the community to adopt a confusing and frustrating way of organizing code.

There are better ways to organize code. We just need to look at how other languages have solved this problem and learn from their mistakes.
